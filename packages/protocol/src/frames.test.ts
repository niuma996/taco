...
describe("protocol version contract (v2 dropped sidecar.hello)", () => {
    it("declares the current protocol version", () => {
        assert.deepEqual(SIDECAR_PROTOCOL_VERSION, { major: 2, minor: 0 });
    });

    it("rejects a major mismatch in either direction", () => {
        assert.equal(isCompatibleSidecarProtocol({ major: 0, minor: 9 }), false);
        assert.equal(isCompatibleSidecarProtocol({ major: 3, minor: 0 }), false);
    });

    it("accepts an equal or newer sidecar minor (additive)", () => {
        assert.equal(isCompatibleSidecarProtocol({ major: 2, minor: 0 }), true);
        assert.equal(isCompatibleSidecarProtocol({ major: 2, minor: 4 }), true);
    });

    it("rejects an older sidecar minor (client may call missing methods)", () => {
        assert.equal(isCompatibleSidecarProtocol({ major: 2, minor: -1 }), false);
    });

    it("rejects a missing or malformed minor", () => {
        assert.equal(isCompatibleSidecarProtocol({ major: 2, minor: undefined }), false);
        assert.equal(isCompatibleSidecarProtocol({ major: 2, minor: "0" }), false);
        assert.equal(isCompatibleSidecarProtocol({ major: 2, minor: null }), false);
    });
});
...
describe("isCompatibleClientProtocol (server-side gate)", () => {
    it("rejects a major mismatch in either direction", () => {
        ...
        assert.equal(isCompatibleClientProtocol({ major: 3, minor: 0 }), false);
    });

    it("accepts an equal or older client minor", () => {
        assert.equal(isCompatibleClientProtocol({ major: 2, minor: 0 }), true);
    });

    it("rejects a newer client minor (server cannot honour unknown methods)", () => {
        assert.equal(isCompatibleClientProtocol({ major: 2, minor: 1 }), false);
        assert.equal(isCompatibleClientProtocol({ major: 2, minor: 4 }), false);
    });

    it("rejects a missing or malformed minor", () => {
        assert.equal(isCompatibleClientProtocol({ major: 2, minor: undefined }), false);
        assert.equal(isCompatibleClientProtocol({ major: 2, minor: "0" }), false);
        assert.equal(isCompatibleClientProtocol({ major: 2, minor: null }), false);
    });
});
...
describe("initialize handshake contract", () => {
    ...
    it("InitializeParams carries protocolVersion + clientCapabilities", () => {
        ...
        assert.equal(params.protocolVersion.major, 2);
        assert.equal(params.protocolVersion.minor, 0);
        assert.equal(params.clientCapabilities.uiLocale, "en");
    });

    it("InitializeResult carries serverVersion, serverCapabilities, protocolVersion + identity", () => {
        const result = {
            serverVersion: "1.0.0",
            serverCapabilities: {
                methods: ["initialize"],
