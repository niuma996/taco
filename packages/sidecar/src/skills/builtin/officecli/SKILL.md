---
name: officecli
description: Use when creating, editing, inspecting, proofreading, or otherwise modifying Office documents (.docx, .xlsx, .pptx). Covers document creation, format inspection (view issues / validate), content extraction (view text / query / get), property edits (set / add / remove / move / batch), raw XML escapes (raw / raw-set), and live PPT/Word preview via watch + browser selection. Requires the `officecli` binary on PATH; install with `curl -fsSL https://d.officecli.ai/install.sh | bash` if missing.
---

<!--
  Vendored from iOfficeAI/OfficeCLI (Apache-2.0).
  Upstream: https://github.com/iOfficeAI/OfficeCLI/blob/main/skills/officecli/SKILL.md
  Body kept verbatim from upstream so vocabulary, prop names, and command syntax
  stay in lock-step with the binary. Frontmatter description rewritten to match
  taco's "Use when …" trigger convention; body content is owned by officecli.
-->
