...
| **Handshake (v2+)** | Mandatory `initialize` RPC; the server rejects all non-`initialize` calls with `not_initialized` until the handshake completes. The `initialize` response carries `serverVersion`, `pid`, `instanceId`, and `protocolVersion` (identity previously delivered by the retired `sidecar.hello` push frame). See `docs/sidecar-protocol.md` for the full contract. |
